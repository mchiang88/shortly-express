...
app.post('/signup',
  function (req, res) {
    ...
    new User({ username }).fetch().then(function (found) {
      if (found) {
        res.status(200).send('user already exists');
      } else {
        //how do we store these info securely? 
        Users.create({
          username: username,
          password: password
        })
...
app.post('/login',
  function (req, res) {
    ...
    new User({ username, password }).fetch().then(function (found) {
      if (found) {
        if (found.attributes.username === username && found.attributes.password === password) {
